...
export const Dashboard: React.FC<{ children: React.ReactNode }> = ({
  ...
  const currentCollection = Array.from(LINKS.entries()).find(
    ([, collectionPage]) =>
      !!collectionPage.find((link) => location.pathname.includes(link.path))
  );

  const currentPageName =
    currentCollection &&
    `${currentCollection[0].label} ${
      currentCollection[1].find((link) => location.pathname.includes(link.path))
        ?.text
    }`;
